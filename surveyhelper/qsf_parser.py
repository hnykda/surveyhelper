...
class QsfParser:
    ...
    def create_questions(self):
        ...
        questions = []
        for qid in qids:
            json = qid_to_json[qid]['Payload']
            selector = json['Selector']
            qtype = json['QuestionType']
            if (qtype == 'MC' and selector in ['SAVR', 'SAHR', 'DL', 'SACOL']):
                questions.append(self.build_sqsr(json))
            elif (qtype == 'MC' and selector in ['MACOL', 'MAVR']):
                questions.append(self.build_sqmr(json))
            elif (qtype == 'Matrix' and selector == 'Likert' and json['SubSelector'] == 'SingleAnswer'):
                questions.append(self.build_mqsr(json))
            elif qtype == 'Slider':
                questions.append(self.build_slider_mqsr(json))
            elif (qtype == 'Matrix' and selector == 'Likert' and json['SubSelector'] == 'MultipleAnswer'):
                questions.append(self.build_mqmr(json))
            else:
                txt = json['QuestionText']
                logging.info("Skipping question %s\n\nType: %s\nSelector: %s\nText: %s",
                             qid, qtype, selector, txt[:100])
    ...
    def get_proper_text(self, choices_display):
        if 'Image' in choices_display.keys():
            text = choices_display['Image']['Display']
        else:
            text = choices_display['Display']
        return text

    def build_sr_question_from_id(self, id, json, tag, recode, exclude):
        text = self.get_proper_text(json['Choices'][str(id)])
